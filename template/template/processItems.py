...
# redis数据库链接
redis_client = redis.StrictRedis(host="redis", port=6379,password='caozongchao',db=0)
# mysql数据库链接
mysql_client = Db()
...
while True:
    print('插入第%d条' % i)
    source, data = redis_client.blpop(["cs:items"])
    item = json.loads(data.decode())
    sql = "insert into items(title,link,body) values(%s,%s,%s)"
    params = [item["title"], item["link"], item["body"]]
