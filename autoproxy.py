...
class AutoProxyMiddleware(object):
    ...
    def process_request(self, request, spider):
        ...
        if self.len_valid_proxy() > 0:
            ...
            if 'proxy' in request.meta:
                del request.meta['splash']['args']['proxy']
    ...
    def process_respose(self, request, response, spider):
        ...
        if response.status in self.ban_code:
            self.invaild_proxy(request.meta['splash']['args']['proxy'])
            logger.debug("Proxy[%s] ban because return httpstatuscode:[%s]. ", request.meta['splash']['args']['proxy'], str(response.status))
        ...
        if self.re:
            ...
            if match:
                self.invaild_proxy(request.meta['splash']['args']['proxy'])
                logger.debug("Proxy[%s] ban because pattern match:[%s]. ", request.meta['splash']['args']['proxy'], str(match))
                new_request = request.copy()
                new_request.dont_filter = True
                return new_request

        p = request.meta['splash']['args']['proxy']
    ...
    def process_exception(self, request, exception, spider):
        if isinstance(exception, self.EXCEPTIONS_TO_CHANGE) \
            ...
            self.invaild_proxy(request.meta['splash']['args']['proxy'])
            logger.debug("Proxy[%s] connect exception[%s].", request.meta['splash']['args']['proxy'], exception)
            new_request = request.copy()
            new_request.dont_filter = True
            return new_request
    ...
    def set_proxy(self, request):
        ...
        request.meta['splash']['args']['proxy'] = self.proxy[self.proxy_index]
    ...
    def check_proxy(self, proxy):
        ...
        try:
            for url, code in self.autoproxy.test_urls:
                resbody = requests.get(url, proxies=proxies,timeout=self.autoproxy.test_proxy_timeout).text
